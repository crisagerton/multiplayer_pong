...
vector<int> webSocket::getClientIDsWithSamePortAs(int clientID) {
	vector<int> clientIDS;
	int clientPort = portClientMap[clientID];
	for (pair<int, int> portClientPair : portClientMap) {
		if (portClientPair.second == clientPort) {
			clientIDS.push_back(portClientPair.first);
		}
	}
	return clientIDS;
}
...
void webSocket::wsAddClient(int socket, in_addr ip, int port) {
	FD_SET(socket, &fds);
	if (socket > fdmax)
		fdmax = socket;
...
void webSocket::startServers(int ports[]) {
	showAvailableIP();

	int yes = 1;
	char buf[4096];
	struct sockaddr_in serv_addr, serv_addr2, serv_addr3, serv_addr4, cli_addr;
	...
	int listenfd2, listenfd3, listenfd4;
	...
	serv_addr.sin_port = htons(ports[3]);
	listenfd4 = socket(AF_INET, SOCK_STREAM, 0);
	if (setsockopt(listenfd4, SOL_SOCKET, SO_REUSEADDR, (const char *)&yes, sizeof(int)) == -1) {
		perror("setsockopt() error!");
		exit(1);
	}
